<?php
# https://ariefsan.crewbasproject.my.id/telegram/execution-3/other/other-07-generateElement.php
require '../../00-01-conn-agent.php';
require '../../00-03-base-config.php';
$config = config();

# file_get_contents("https://api.telegram.org/bot5217628574:AAHpzjY9JAcvS5Dg67UxJdwFz0-EQ2tisUA/sendMessage?chat_id=474974312&text=03-01-ticket-entry.php-line13");

$json = json_decode($_POST['let2']);
#$sender = $_POST['let1'];
$depo = $json[1];
#$cutdate = $json[2];

#$depocode = '70002157';
#$cutdate = '2023-05-05';]

$sql1 = "set @rowNumv = 0;";
$sql2 = "select aa.*
from(
	select 
	@rowNumv := @rowNumv + 1 as rownumber 
	,p.product_id 
	, p.product_code 
	, p.product_name 
	, p.short_name 
	, p.is_active 
	, (select pd.cbp from product_depo pd join depo d on d.depo_id=pd.depo_id where d.depo_code = '$depo' and pd.product_id=p.product_id and pd.is_active=1) as cbp
	from product p 
	# left join product_depo pd on pd.product_id = p.product_id 
	where 0=0
	order by p.crate_id, p.subbrand_id 
) as aa 
having cbp is not null
";

$exec1 = mysqli_query($conn, $sql1) or die(mysqli_error($conn));
$exec2 = mysqli_query($conn, $sql2) or die(mysqli_error($conn));

$dataMessage = "";

if(mysqli_num_rows($exec2) > 0) {
    
	while($row= mysqli_fetch_array($exec2)) {
      	# "{'check' : '/html/body/main/div[4]/div[1]/div/div/table/tbody/tr[", aa.rownumber, "]/td[1]/p/label', 'price' : '", aa.cbp, "', 'id' : 'cbp-", aa.rownumber, "', 'code' : '", aa.product_code, "'},") as string
      	
      	$dataMessage .= "{'check' : '/html/body/main/div[4]/div[1]/div/div/table/tbody/tr[". $row['rownumber']. "]/td[1]/p/label', 'price' : '". $row['cbp']. "', 'id' : 'cbp-". $row['rownumber']. "', 'code' : '". $row['product_code']. "'},\n" ;
      
      
      	#$dataMessage .= $row['depo_code']. "-". $row['depo_name']. "-". $row['RITASE']. " " . "(". $row['last_po']. ")\n" ;

      	# Prepare Product untuk menggunakan product_code, sebagai bantuan untuk sortir nanti
		#$customProd = $row['product_code']. "-". $row['short_name'];
      
      	# Masukkan list semua product kedalam variable dataHeader
      	#array_push($dataHeader, $customProd);
        
      	#$tempDataRow = $row['store_id']. "$". $row['store_name'];
      	# Masukkan store_id kedalam object dataBody, dengan sifat unique/distinct
      	#$dataBody += [ $tempDataRow  => array() ];
      	
      	# Setelah toko sdh di insert di object, maka toko ini juga diinsert array (multidimensi)
      	#$dataBody[$tempDataRow] += [$customProd => array($row['qty_estimasi'], $row['qty_dropping'], $row['qty_sold'], $row['qty_bs']) ];
      
      }
  
	# $config["whatsappSendMessage"]($config['key-wa-bas'], $dataMessage , $sender, "true");
}
  
# print($dataMessage);
# "whacenterSendGroupMessage" => function ($key, $groupName, $message)
$config["whacenterSendGroupMessage"]($config['key-whacenter-1'],  "Crew BAS", $dataMessage);
# $filename = substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'),1 , 4). "_topjwkissue.txt";

# print($msd);
# $writeResult = file_put_contents("download/". $filename, $dataMessage);

# if ($writeResult != false) {
  # $config["whatsappSendMessage"]($config['key-wa-bas'],  "Filter Warung TOP 7, JWK Lebih dari 1", $config['id-wa-group-fa'], "true");
  # $config["whatsappSendDocs"]($config['key-wa-bas'],  $config['id-wa-group-fa'], "https://ariefsan.crewbasproject.my.id/telegram/execution-3/report/download/". $filename,  "true");
# }





